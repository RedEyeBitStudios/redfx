#include "renderui_textbox.hpp"

using ClassImpl = nxcraft::intern::subsystems::GPGPU_ProcessorStageResources_RenderUI_TextBox;
...
ClassImpl::GPGPU_ProcessorStageResources_RenderUI_TextBox(const GPGPU_Device_Vulkan::Commons& commons, const GPGPU_ProcessorStageResources_RenderUI_General& renderui_generals)
{
	...
	const std::vector<VkVertexInputAttributeDescription> vertex_attribs_info
	{
	...
	const VkPipelineVertexInputStateCreateInfo vertex_input_info
	{
		...
		.pNext = nullptr,
		.flags = 0,
		.vertexBindingDescriptionCount = static_cast<uint32_t>(vertex_bindings_info.size()),
		.pVertexBindingDescriptions = vertex_bindings_info.data(),
		.vertexAttributeDescriptionCount = static_cast<uint32_t>(vertex_attribs_info.size()),
		.pVertexAttributeDescriptions = vertex_attribs_info.data()
	};
