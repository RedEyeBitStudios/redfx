...
flat layout(location = 2) in f16vec4 color;
...
void main()
{
	const vec3 color_midprec = vec3(color.rgb);
	color_attachment = vec4(color_midprec, texture(images[uint(texture_id)], vec2(texture_coords)).r * float(color.a));
}